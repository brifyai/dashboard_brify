-- MIGRACIÓN CORREGIDA PARA SUPABASE
...
-- PASO 2: Si no existe, crearlo manualmente (con UUID correcto)
-- Primero obtener el UUID real del usuario desde auth.users
SELECT id, email FROM auth.users WHERE email = 'camiloalegriabarra@gmail.com';

-- PASO 3: Insertar o actualizar usando el UUID correcto del auth.users
INSERT INTO users (
  id,
  email,
  name,
...
SELECT 
  id,
  email,
  COALESCE(raw_user_meta_data->>'full_name', 'Camilo Alegria'),
  'active',
  NOW(),
  NOW()
FROM auth.users 
